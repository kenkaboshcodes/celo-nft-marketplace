...
// burn or delete an NFT from marketplace
export const deleteNft = async (minterContract, performActions, tokenId) => {
  await performActions(async (kit) => {
    try {
      console.log('delete')
      await minterContract.methods
        .deleteNft(tokenId)
        .send({ from: kit.defaultAccount });
      toast(<NotificationSuccess text="Deleted NFT Succesfully." />);
    } catch (error) {
      console.log(error);
    }
  });
